...
from eagerx_franka.panda_ros.pose_transform_functions import  array_quat_2_pose, list_2_quaternion, pose_2_transformation, position_2_array, orientation_2_quaternion, array_array_2_pose
...
class Panda():
    ...
    def home(self):
        pos_array = np.array([.4, -0.05,  0.25]) # home position
    ...
    def joint_states_callback(self, data):
        ...
        # self.curr_joint_vel = data.velocity[:7]
    ...
    def go_to_pose(self, goal_pose, do_spiral_search=True, interp_dist=0.001, interp_dist_polar=0.001):
        if self.goal_pose is not None:
           if self.goal_pose.pose.position.x == goal_pose.pose.position.x and self.goal_pose.pose.position.y == goal_pose.pose.position.y and self.goal_pose.pose.position.z == goal_pose.pose.position.z:
               if self.goal_pose.pose.orientation.x == goal_pose.pose.orientation.x and self.goal_pose.pose.orientation.y == goal_pose.pose.orientation.y and self.goal_pose.pose.orientation.z == goal_pose.pose.orientation.z and self.goal_pose.pose.orientation.w == goal_pose.pose.orientation.w:
        ...
        self.goal_updated = False
        self.go_to_pose_thread = Thread(target=self._go_to_pose, args=(goal_pose, do_spiral_search, interp_dist, interp_dist_polar))
    ...
    def _go_to_pose(self, goal_pose, do_spiral_search=True, interp_dist=0.001, interp_dist_polar=0.001): 
        ...
        for i in range(step_num):
            if self.goal_updated:
                # print("new pose, so aborting old plan")
                break
